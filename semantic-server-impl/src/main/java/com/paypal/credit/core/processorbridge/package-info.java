 ...
 * a client facade (REST, AMQP,SOAP, JMS, etc...) and the command providers and processor.
 *
 * The processor bridge declaration is simply an interface which is used to generate
 * code to call or to proxy calls to the command providers and the command processor.
 * Methods in the processor bridge must be constrained to the Application Semantics
 * and the Application Domain Model. The application domain model must be available,
 * for class resolution, when the processor bridge is generated (or proxied).
 *
